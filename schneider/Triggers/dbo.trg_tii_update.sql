...
GO
CREATE trigger [dbo].[trg_tii_update] on [dbo].[tii_ticketInfo] for update
as
begin
	if TRIGGER_NESTLEVEL() = 1
	BEGIN
		PRINT 'tii_ticketinfo - update trigger'
		declare cur_trg_tii_update cursor for (select unid, ti_ticket_UNID, atl_artikel_UNID, cs1_cos1_UNID, cs2_cos2_UNID, cs3_cos3_UNID, tii_bulkinsert from tii_ticketInfo)
		declare @u int
		declare @ti int
		...
		open cur_trg_tii_update

		fetch next from cur_trg_tii_update into @u, @ti, @atl, @cos1, @cos2, @cos3, @bulk
		...
		close cur_trg_tii_update
		deallocate cur_trg_tii_update
	end
end
GO
